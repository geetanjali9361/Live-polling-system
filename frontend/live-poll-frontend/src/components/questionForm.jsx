...
export default function QuestionForm({ onCreated }) {
  ...
  return (
    <div className="panel">
      ...
      <input value={title} onChange={e=>setTitle(e.target.value)} placeholder="e.g., Solar System Quiz" />
      ...
      <textarea value={text} onChange={e=>setText(e.target.value)} placeholder="Which planet is known as the Red Planet?" />
      <div>
        <h4>Edit Options</h4>
        {opts.map((v,i)=>(
          <div key={i} className="row">
            <input value={v} onChange={e=>setOpt(i, e.target.value)} placeholder={`Option ${i+1}`} />
            <label>
              <input type="radio" checked={Number(correct)===i} onChange={()=>setCorrect(i)} />
              Correct
            </label>
          </div>
        ))}
        <button onClick={addOpt}>+ Add More option</button>
      </div>
